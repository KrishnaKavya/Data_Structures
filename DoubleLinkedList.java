...
public class DoubleLinkedList {
	 ...
	 * the value is calculated when the given parameter does not have any characters . 
	 * else it returns -1. 
	 * 
	...
	public static void main(String[] args) {
		...
		DoubleLinkedList obj = new DoubleLinkedList(sc.nextInt());
		...
		do {
			...
			switch (ch) {
			case "1":
				System.out.println("Please enter the value of element to insert into the DLL");
			...
			case "2":
				 ...
				 * 
				 */
				int del = (input1.length() == 1 ? (Character.isDigit(input1.charAt(0)) ?
						Integer.valueOf(input1) : input1.charAt(0)) : obj.checkInteger(input1));
				if (del != -1) {
					boolean success=obj.delete(del);
					if(!success){
						System.out.println("The element entered is not in the DLL.");
					}
				} else {
					System.out.println(" The value entered is not an integer.");
				}
